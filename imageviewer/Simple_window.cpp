...
Simple_window::Simple_window(Point xy, int w, int h, const string& title) :
    ...
    submit_button(Point(x_max()-400, 20), 85, 35, "Submit", cb_submit),
    browse_button(Point(x_max()-400, 60), 85, 35, "Browse", cb_browse),
    input_box1 (Point(400, 20), 400, 30, "File Location"),
    rect_back(Point(0, 0), x_max(), y_max()), //black background
    previous_button(Point(x_max()/4, y_max()- 80), 300, 80, "Previous", cb_previous),
    next_button(Point(x_max()-(x_max()/2), y_max()- 80), 300, 80, "Next", cb_next),
    delete_button(Point(0, y_max() - 50), 115, 50, "Delete", cb_deleted),
    tag1_button(Point(x_max() - 130, y_max() - 245), 120, 45, "Add to Family", cb_tag1),
    tag2_button(Point(x_max() - 130, y_max() - 290), 120, 45, "Add to Friends", cb_tag2),
    tag3_button(Point(x_max() - 130, y_max() - 335), 120, 45, "Add to Aggieland", cb_tag3),
    tag4_button(Point(x_max() - 130, y_max() - 380), 120, 45, "Add to Pets", cb_tag4),
    tag5_button(Point(x_max() - 130, y_max() - 425), 120, 45, "Add to Vacation", cb_tag5),
    
     tag1_switch_button(Point(90, y_max() - 245), 120, 45, " View Family", cb_switch_tag1),
    tag2_switch_button(Point(90, y_max() - 290), 120, 45, "View Friends", cb_switch_tag2),
    tag3_switch_button(Point(90, y_max() - 335), 120, 45, "View Aggieland", cb_switch_tag3),
    tag4_switch_button(Point(90, y_max() - 380), 120, 45, "View Pets", cb_switch_tag4),
    tag5_switch_button(Point(90, y_max() - 425), 120, 45, "View Vacation", cb_switch_tag5),
...
{
if (img) detach(*img); // removes image if it is currently attached
 	 --pic_view;
    if (vr.size() > pic_view) {
	    check_tag_status();
        delete img; 
       if (vr[pic_view ].find(',') != string::npos)
        ...
        else  {img = new Image(Point (x_max()-950, y_max()/7), vr[pic_view]);}
...
void Simple_window::browse()
{
    ...
    string filename = filelocation.substr(filelocation.find_last_of('/') + 1, string::npos); //substr such that only the name of the file is obtained
    string cp_string = "if [ ! -f filename] then cp " + filelocation + " " + filename; //command to be used in next line if file doesnt already exist.
...
{
    if (vr.size() > 0 ) { //make sure it has something to delete
        ...
        if (img) {detach(*img); delete img; }//remove old image that is being displayed
        if (pic_view < vr.size()) {
            pic_view  = 0;
...
if (b_tag_check == false) return  true;
else if (b_tag_check == true) return  false;
}
...
deque<string> tag_options = {"vacation", "pets", "aggieland", "friends", "family"  };
...
case 8: b_tag9 = true;
case 9: b_tag10 = true; }
}else {vr_rect[i].set_color(Color::red); {
switch(i)  //used to change the value of the tag bool so that proper color is displayed.
{
case 5: b_tag6 = false;
case 6: b_tag7 = false;
...
void Simple_window::color_change(int col, bool &b_tag_check) {
  ...
  else  { vr_rect[col].set_color(Color::red); redraw();}
}
